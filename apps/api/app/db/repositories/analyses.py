...
async def get_analysis(analysis_id: str, user_id: str) -> dict | None:
    client = await get_client()
    res = await client.table("analyses").select("*").eq("id", analysis_id).eq("user_id", user_id).single().execute()
    return res.data


async def list_analyses(user_id: str, limit: int = 20, offset: int = 0, status: str | None = None) -> tuple[list[dict], int]:
    client = await get_client()
    query = client.table("analyses").select("id, status, input, created_at, completed_at, duration_ms", count="exact").eq("user_id", user_id)
    if status:
...
async def update_analysis_status(analysis_id: str, status: str, result: dict | None = None, error: str | None = None, duration_ms: int | None = None) -> None:
    client = await get_client()
